...
## Permission checks before app-privileged calls
- Every action a user triggers runs with the user's own identity (`asUser()`).
- The app identity (`asApp()`) is used only after the invoking user's **update permission on the target
  content** has been verified through the Confluence content-permission check API (Forge Authorize API):
  "Notify a group" checks the page; creating a sync rule checks the page or the space homepage; "Run sync
  now" checks each rule and skips the rules the user may not update.
- The daily scheduled sync runs without a user and only applies rules created by permission-checked users.

## Permissions (least privilege)
`storage:app`, `read:group:confluence`, `read:user:confluence`, `write:watcher:confluence`,
`write:comment:confluence`, `read:content.permission:confluence`, `read:space:confluence`. Each scope is justified on the Marketplace Privacy & Security tab. Scheduled
